-- New-13-Fix-Turkish-Encoding.sql
-- Problem: cities/districts names show mojibake in UI (e.g. "??stanbul" instead of "İstanbul").
-- Root cause: when 02-seed-data.sql (correct UTF-8) was imported, each non-ASCII byte was
--   replaced with a literal '?' (0x3f). Each 2-byte Turkish char became "??". The original
--   letters are LOST in the DB, so this cannot be recovered by re-decoding — names must be rewritten.
-- 31/81 cities and 509/975 districts are corrupted.
-- Fix strategy (no data loss, IDs preserved so the 3 salon→district FKs stay intact):
--   A) Cities: re-apply canonical names keyed on plate_code (UPSERT). cities.id unchanged.
--   B) Districts: rebuild correct names into a TEMP table, then UPDATE-match each corrupted row
--      by (city_id, corrupted_form) where corrupted_form = each Turkish char replaced by '??'.
-- Idempotent: re-running is a no-op once names are correct.

BEGIN;

-- ============================================================
-- PART A — Cities (keyed on plate_code)
-- ============================================================
...
-- ============================================================
-- PART B — Districts (match corrupted rows, preserve IDs)
-- ============================================================
CREATE TEMP TABLE tmp_correct_districts (city_id uuid, name text) ON COMMIT DROP;

INSERT INTO tmp_correct_districts (city_id, name)
...
UPDATE districts d
SET name = cd.name
FROM tmp_correct_districts cd
WHERE d.city_id = cd.city_id
  AND d.name = regexp_replace(cd.name, '[çÇğĞıİöÖşŞüÜâÂîÎûÛ]', '??', 'g')
  AND d.name <> cd.name
  AND NOT EXISTS (
    SELECT 1 FROM districts d2
    WHERE d2.city_id = d.city_id AND d2.name = cd.name
  );
