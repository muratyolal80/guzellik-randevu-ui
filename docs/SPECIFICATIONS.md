...
| 1.1.5 | 29.06.2026 | **Mobil navigasyon + müşteri UX + onay akışı uzlaştırma + çok sayıda DB/RLS düzeltmesi** — (a) **Mobil nav (yeni)**: müşteri/owner/admin panellerinin sidebar'ı `lg` altında gizliydi ve alternatifi yoktu → üç panele de sabit **alt tab bar** eklendi (safe-area padding + içerik alt boşluğu); admin sidebar mobilde gizlendi. (b) **DB migration'ları**: `New-26-Salon-Images` (salon-images storage RLS — owner kapak/galeri yükleme), `New-27` (salon_usage_stats view STARTER COALESCE fallback — galeri 0/0 hatası), `New-28` (`subscriptions.billing_cycle` eksik kolon — paket kaydı schema-cache hatası), `New-29` (salon_type_categories seed — "Standart Hizmetleri Toplu Ekle" kategori boşluğu; AI değil DB), `New-30` (`salons` SUPER_ADMIN/ADMIN tam erişim RLS — admin onay kuyruğu sadece APPROVED görüyordu), `New-31`+`New-32` (randevu onay izi `approved/cancelled_by`; ardından `cancelled_by` FK `auth.users`→`profiles` uzlaştırma — "Could not find relationship appointments↔profiles" hatası). (c) **Onay akışı uzlaştırma**: iki paralel implementasyon (approved_by vs confirmed_by) → `updateAppointmentStatus` kanonik `confirmed_by/completed_by/cancelled_by` şemasına hizalandı (quick-action onayları da modaldaki "kim/ne zaman" izinde görünür). (d) **Owner Randevular liste/sorgu ekranı (yeni)** `/owner/appointments`: tarih aralığı + durum (adetli çipler) + personel + arama filtreli tablo, hızlı onay/iptal + detay. (e) **Akıllı Takvim pro UX**: durum-bazlı renk + tıklanabilir durum çipleri (adetli), gün/hafta/ay aktif düzeltmesi, gün başlığı/saat kolonu renkleri, görünen-aralığa-göre fetch (Temmuz randevuları gelmiyordu), boş slota çift-tık ekleme, iptal slotuna "Bu Saate Yeni Müşteri". (f) **Admin salon edit pariteleştirme** `/admin/salons/[id]/edit`: eksik alanlar (telefon/kapı no/sokak/daire/tip/açıklama) + şehir/ilçe/adres değişince otomatik konum + manuel enlem/boylam. (g) **Düzeltmeler**: middleware `.maybeSingle()` çok-salonlu owner'ı onboarding'e sektiriyordu (tüm /owner 307→redirect bug); `sendNotification` yanlış kolon (`message/action_url`→`content/link`, "{}" hatası); `AuditLogService.logAction` salon_id yoksa atla (müşteri doğum tarihi "{}" hatası); şube limiti pasif/taslak salonu sayıyordu; `adminAssignSubscription` salon_id'siz orphan abonelik → owner'ın salonuna bağlanıyor; UserMenu sentetik `@pending.user` email yerine isim/telefon; BulkAddServicesModal/BranchSelector etiket düzeltmeleri. (h) **Müşteri UX**: Ayarlar menüye eklendi, gerçek logout, favori "Randevu Al" linki, bildirim aksiyonları mobilde görünür, dashboard öneri boş-durumu, payments yanıltıcı statik bakiye kaldırıldı, reviews salon linki sadeleştirildi. tsc 0 hata |
| 1.1.4 | 28.06.2026 | **Randevu aksiyon izi + Owner takvim detay modal pro UX** — (a) `appointments` tablosuna 7 yeni kolon: `confirmed_by/at`, `completed_by/at`, `cancelled_by/at`, `cancellation_reason` (hepsi `profiles(id)` FK ON DELETE SET NULL, partial index'li) — `audit_logs`'a ek olarak hızlı UI render için satır içi izleme; migration `New-26-Appointment-Action-Tracking.sql` (mevcut `audit_logs`'tan backfill dahil); (b) `/api/appointments/[id]/confirm` ve `/reject` route'ları aksiyon kolonlarını set ediyor; **yeni** `/api/appointments/[id]/complete` endpoint'i CONFIRMED → COMPLETED veya `{isNoShow:true}` ile NO_SHOW geçişini RBAC kontrolüyle (owner/atanmış staff/admin) yapıyor; (c) Owner takvim (`/owner/calendar`) detay modal'ı baştan yazıldı: durum-temalı header (5 renk + 5 ikon: Hourglass/CheckCircle2/UserCheck/XCircle/SkipForward), tıkla-ara/kopyala/WhatsApp olan müşteri kartı, hizmet+uzman kartları, indirim+kapora ayrıştırılmış ödeme kırılımı, Aktivite Geçmişi (kim/ne zaman: oluşturuldu→onayladı→tamamladı/iptal etti — profil isim join'i ile), durum-aware aksiyon footer'ı (PENDING→Onayla/Reddet, CONFIRMED→Tamamlandı/Gelmedi/İptal, CANCELLED→Bu Saate Yeni Müşteri), Yazdır butonu; (d) `types.ts` `Appointment` arayüzü `NO_SHOW` status + tüm aksiyon kolonlarını taşıyacak şekilde güncellendi; (e) takvim boş slot **çift tıklayınca** o saat+seçili personelle Yeni Randevu modal açılıyor (preselect). 90/90 vitest, tsc 0 hata |
| 1.1.3 | 28.06.2026 | **Destek modülü müşteri UI zenginleştirme** (DB değişikliği yok) — `/customer/support` ve `/customer/support/[id]` profesyonelleştirildi: (a) özet istatistik kartları (Toplam/Açık/İşleniyor/Çözüldü); (b) durum sekmeleri + kategori filtresi + arama; (c) **öncelik UI** — oluşturma formunda seçim + listede rozet/sol vurgu (öncelik önceden sadece `support_tickets.priority` kolonundaydı, UI'da yoktu); (d) form validasyonu (min uzunluk + karakter sayacı) + `useToast` (native `alert`/`confirm` kaldırıldı); (e) Skeleton yükleme + hata/boş durum ayrımı; (f) detay sayfasında iyimser mesaj gönderimi + inline "Çözüldü" onayı; (g) ortak `lib/support.ts` helper (durum/öncelik/kategori config + `formatRelativeTime`, 3 yerdeki tekrarı kaldırır). `SupportService.createTicket`'e `priority` parametresi eklendi. tsc 0 hata, lint 0 hata. Belge: `docs/modules/support.md` güncellendi |
