...
export const HomePage: React.FC = () => {
  ...
  const lat = Number(mapCenterRaw?.lat);
  const lng = Number(mapCenterRaw?.lng);
  ...
  const handleSuggestionSelect = (item: typeof suggestions[0]) => {
      ...
      // Add mode param
      if (item.type === 'salon') query += `&mode=salon`;
      else if (item.type === 'category') query += `&type=${item.id}`; // Categories usually use 'type' param
      else if (item.type === 'service') query += `&mode=service`;
                                ...
                                return (
                                    <Marker 
                                        ...
                                        eventHandlers={{
                                            ...
                                            click: () => window.location.href = `#/salon/${salon.id}`
                                        }}
                                    >
          ...
          <div className="w-full max-w-4xl bg-white rounded-xl shadow-2xl p-2 mx-auto border border-white/20 backdrop-blur-sm relative z-50 animate-fade-in-up">
              ...
              <div className="px-4 py-2 flex items-center justify-start gap-2 text-xs text-text-secondary border-t border-gray-50 mt-1 pt-3">
                  ...
                  {CATEGORIES.slice(0, 3).map(cat => (
                       <Link key={cat} to={`/?type=${cat}`} className="hover:text-primary hover:underline font-medium">{cat}</Link>
                  ))}
              </div>
          </div>
        </div>
          ...
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {popularServices.map((service, idx) => (
                      <Link 
                        key={idx} 
                        to={`/?search=${encodeURIComponent(service)}`}
