...
const ReviewService = {
   ...
   */ async getReviewableAppointments (userId, salonId) {
        ...
        const { data: appointments, error: apptError } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('appointments').select('*').eq('customer_id', userId).eq('salon_id', salonId).eq('status', 'COMPLETED').order('end_time', {
            ascending: false
        });
        ...
        const { data: reviews, error: reviewError } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('reviews').select('appointment_id').eq('user_id', userId).eq('salon_id', salonId).not('appointment_id', 'is', null);
...
const StaffReviewService = {
   ...
   */ async createStaffReview (review) {
        const { data, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('staff_reviews').insert(review).select().single();
        if (error) throw error;
        return data;
    },
    /**
...
const SubscriptionService = {
   ...
   */ async checkLimit (salonId, resourceType) {
        ...
        if (resourceType === 'staff') {
            ...
            // Branches are salons with same owner/group? Currently 1 salon = 1 entity. 
            // Future: max_branches check
...
//# sourceMappingURL=%5Broot-of-the-server%5D__699560fd._.js.map