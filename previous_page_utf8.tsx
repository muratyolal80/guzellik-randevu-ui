        ...
        loading: () => (
            <div className="h-full w-full flex items-center justify-center bg-gray-100">
                <div className="text-center">
                    ...
                    <p className="text-text-muted text-sm">Harita y├╝kleniyor...</p>
                </div>
            </div>
        )
    }
...
const getServiceIcon = (serviceName: string) => {
    ...
    if (lower.includes('sa├ğ') || lower.includes('f├Ân') || lower.includes('kesim') || lower.includes('r├Âfle') || lower.includes('ombre')) return 'content_cut';
    if (lower.includes('t─▒rnak') || lower.includes('oje') || lower.includes('manik├╝r') || lower.includes('pedik├╝r') || lower.includes('jel')) return 'brush';
    if (lower.includes('makyaj') || lower.includes('kirpik') || lower.includes('ka┼ş') || lower.includes('lifting')) return 'face';
    if (lower.includes('masaj') || lower.includes('spa')) return 'spa';
    if (lower.includes('cilt') || lower.includes('bak─▒m') || lower.includes('peeling')) return 'clean_hands';
    if (lower.includes('lazer') || lower.includes('epilasyon') || lower.includes('a─şda')) return 'flash_on';
    if (lower.includes('sakal') || lower.includes('t─▒ra┼ş')) return 'face_retouching_natural';
    if (lower.includes('solaryum')) return 'light_mode';
    if (lower.includes('diyet') || lower.includes('zay─▒flama')) return 'monitor_weight';
    return 'star';
};
...
function HomePageContent() {
    ...
    const [selectedCity, setSelectedCity] = useState(cityParam || 'T├╝m├╝');
    const [selectedDistrict, setSelectedDistrict] = useState('T├╝m├╝');
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                console.log('­şöä Fetching Front Page Data...');
                ...
                console.log('Ô£à Base Data Fetched:', {
                ...
                const mappedData = (salonsData || []).map((salon: any) => ({
                    ...salon,
                    city: salon.city_name || salon.cities?.name || salon.city?.name || 'Belirtilmemi┼ş',
                ...
                console.log('Ô£à Data Mapped Successfully:', mappedData.length, 'salons');
                ...
                console.log('­şöä Loading services for', salonsData?.length, 'salons...');
                ...
                await Promise.all(
                    (salonsData || []).map(async (salon) => {
                        try {
                            ...
                            console.error(`ÔØî Error loading services for salon ${salon.id}:`, error);
                            servicesMap[salon.id] = [];
                        }
                    })
                );
                setSalonServicesMap(servicesMap);
                console.log('Ô£à Services Map Loaded');

            } catch (err: any) {
                console.error('ÔØî Error fetching initial data details:', {
    ...
    useEffect(() => {
        const loadDistricts = async () => {
            if (selectedCity && selectedCity !== 'T├╝m├╝') {
    ...
    useEffect(() => {
        ...
        if (selectedCity !== 'T├╝m├╝' && selectedDistrict !== 'T├╝m├╝' && !availableDistricts.includes(selectedDistrict)) {
            setSelectedDistrict('T├╝m├╝');
        }
    ...
    const filteredSalons = salons.filter(salon => {
        // 1. City Filter
        if (selectedCity && selectedCity !== 'T├╝m├╝' && selectedCity !== 'all') {
        ...
        // 2. District Filter
        if (selectedDistrict && selectedDistrict !== 'T├╝m├╝' && selectedDistrict !== 'all') {
        ...
        if (typeParam && typeParam !== 'all') {
            ...
            const isMatch = salonTypeSlug === typeSlug ||
                ...
                (typeSlug === 'kuafor' && salonTypeName.includes('kuaf├Âr')) ||
                (typeSlug === 'sac' && salonTypeName.includes('sa├ğ'));
    ...
    const handleSuggestionSelect = (item: typeof suggestions[0]) => {
        ...
        if (item.type === 'category' && item.id) {
            ...
            if (selectedCity !== 'T├╝m├╝') query += `&city=${selectedCity}`;
            router.push(query);
            return;
        }
                        ...
                        <div className="p-5 border-b border-border bg-white sticky top-0 z-10 shadow-sm">
                            <div className="relative mb-3" ref={searchWrapperRef}>
                                ...
                                <input
                                    ...
                                    placeholder="Salon, hizmet veya b├Âlge ara..."
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="relative">
                                    ...
                                    >
                                        <option value="T├╝m├╝">T├╝m ┼Şehirler</option>
                                ...
                                <div className="relative">
                                    ...
                                    >
                                        <option value="T├╝m├╝">T├╝m ─░l├ğeler</option>
                            ...
                            <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-50">
                                <span className="text-xs font-bold text-text-main">{filteredSalons.length} i┼şletme bulundu</span>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <Link href="/" onClick={() => { setLocalSearch(''); setSelectedCity('T├╝m├╝'); }} className="text-xs text-primary font-bold hover:underline">Temizle & ├ç─▒k</Link>
                                </div>
                            </div>
                        </div>
                            ...
                            {loading ? (
                                ...
                                <div className={viewMode === 'wide' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6' : 'space-y-5'}>
                                    {visibleSalons.map(salon => (
                                        ...
                                        >
                                            <div className={`relative shrink-0 rounded-xl overflow-hidden bg-gray-100 ${viewMode === 'wide' ? 'w-full aspect-[4/3] mb-4' : 'w-32 h-32'}`}>
                                                ...
                                                {salon.is_sponsored && <div className="absolute top-2 left-2 bg-white/90 backdrop-blur-sm text-primary text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider shadow-sm">├ûne ├ç─▒kan</div>}
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0 flex flex-col justify-between">
                                                ...
                                                <div className="flex justify-between items-end mt-2 pt-2 border-t border-gray-50">
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] text-text-muted font-bold uppercase">Ba┼şlang─▒├ğ</span>
                                                        <span className="text-primary font-bold text-base">{salon.startPrice} Ôé║</span>
                                                    </div>
                                                    <span className="text-xs font-bold text-text-secondary group-hover:text-primary transition-colors flex items-center gap-1">
                                                        ─░ncele <span className="material-symbols-outlined text-sm">arrow_forward</span>
                                                    </span>
                                                </div>
                                            </div>
                                        </Link>
                                    ...
                                    {filteredSalons.length > visibleCount && (
                                        ...
                                        >
                                            Daha Fazla G├Âster <span className="material-symbols-outlined text-lg">expand_more</span>
                                        </button>
                                    )}
                                    ...
                                    {filteredSalons.length === 0 && (
                                        <div className={`flex flex-col items-center justify-center py-20 text-text-muted ${viewMode === 'wide' ? 'col-span-full' : ''}`}>
                                            ...
                                            <p className="font-bold text-text-main">Sonu├ğ bulunamad─▒</p>
                                            <p className="text-sm mt-1">Arama kriterlerinizi de─şi┼ştirerek tekrar deneyin.</p>
                                            <button onClick={() => { setLocalSearch(''); setSelectedCity('T├╝m├╝'); setSelectedDistrict('T├╝m├╝'); }} className="mt-4 text-primary text-sm font-bold hover:underline">Filtreleri Temizle</button>
                                        </div>
                                    )}
                                </div>
                            )}
        ...
        <Layout>
            ...
            {showUnauthorizedError && (
                <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-[9999] animate-fade-in">
                    <div className="bg-red-50 border border-red-200 rounded-lg shadow-lg p-4 flex items-center gap-3 max-w-md">
                        ...
                        <div>
                            <p className="text-red-800 font-bold text-sm">Yetkisiz Eri┼şim</p>
                            <p className="text-red-600 text-xs">Bu sayfaya eri┼şim yetkiniz bulunmamaktad─▒r.</p>
                        </div>
                        <button
            ...
            <div className="relative bg-gray-900 overflow-visible min-h-[550px] lg:min-h-[650px] flex items-center justify-center z-10">
                ...
                <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 flex flex-col items-center text-center mt-10">
                    <span className="inline-block py-1 px-3 rounded-full bg-white/10 border border-white/20 text-white text-xs font-bold tracking-wider uppercase mb-6 backdrop-blur-md">
                        T├╝rkiye'nin #1 Numaral─▒ Platformu
                    </span>
                    <h1 className="text-4xl sm:text-5xl lg:text-7xl font-display font-black text-white mb-6 leading-tight drop-shadow-2xl">
                        G├╝zelli─şin <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-yellow-200 to-primary">Yeni Adresi</span>
                    </h1>
                    <p className="text-lg text-gray-300 mb-10 max-w-2xl font-medium drop-shadow-md leading-relaxed">
                        Size en yak─▒n kuaf├Âr, berber ve g├╝zellik merkezlerini ke┼şfedin, fiyatlar─▒ kar┼ş─▒la┼şt─▒r─▒n ve saniyeler i├ğinde randevunuzu olu┼şturun.
                    </p>
                    ...
                    <div className="w-full max-w-4xl bg-white rounded-xl shadow-2xl p-2 mx-auto border border-white/20 backdrop-blur-sm relative z-50 animate-fade-in-up">
                        ...
                        <div className="flex flex-col md:flex-row gap-2 p-3 bg-white rounded-xl border border-gray-100 shadow-sm relative z-50">
                            ...
                            <div className="flex-grow relative group w-full md:w-2/5" ref={searchWrapperRef}>
                                ...
                                <input
                                    ...
                                    placeholder={
                                        activeTab === 'service' ? 'Hizmet ara (├Ârn. Sa├ğ Kesimi)...' :
                                            activeTab === 'type' ? 'Salon t├╝r├╝ ara (├Ârn. Kuaf├Âr)...' :
                                                'Salon ad─▒ ara...'
                                    }
                                    value={localSearch}
                                    onChange={(e) => handleSearchChange((e.target as HTMLInputElement).value)}
                                ...
                                {showSuggestions && suggestions.length > 0 && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-border rounded-xl shadow-xl z-[100] max-h-80 overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
                                        <div className="p-2 border-b border-gray-50 text-[10px] text-gray-400 font-bold uppercase tracking-wider pl-4">├ûnerilenler</div>
                                        {suggestions.map((item, idx) => (
                                            <div
                                                key={idx}
                                                onClick={() => handleSuggestionSelect(item)}
                            ...
                            <div className="w-full md:w-1/4 relative">
                                ...
                                <select
                                    ...
                                    value={selectedDistrict}
                                    disabled={!selectedCity || selectedCity === 'T├╝m├╝'}
                                    onChange={(e) => setSelectedDistrict((e.target as HTMLSelectElement).value)}
                                >
                                    <option value="T├╝m├╝">T├╝m ─░l├ğeler</option>
                                    {districts.map(d => <option key={d.id} value={d.name}>{d.name}</option>)}
                            ...
                            >
                                ...
                                <span className="md:hidden">Sonu├ğlar─▒ G├Âster</span>
                        ...
                        <div className="px-4 py-2 flex items-center justify-start gap-2 text-xs text-text-secondary border-t border-gray-50 mt-1 pt-3">
                            ...
                            <span className="hidden sm:inline opacity-70">Pop├╝ler: </span>
            ...
            <div className="bg-white py-20 relative z-0">
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-end mb-10">
                        <div>
                            <h2 className="text-3xl font-display font-bold text-text-main mb-2">├ûne ├ç─▒kan Salonlar</h2>
                            <p className="text-text-secondary">Edit├Ârlerimizin sizin i├ğin se├ğti─şi en iyi mekanlar.</p>
                        </div>
                    </div>
                            ...
                            <Link href={`/salon/${salon.id}`} key={salon.id} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-gray-300 hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                                ...
                                <div className="p-5 flex flex-col justify-between flex-1">
                                    ...
                                    <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
                                        <span className="text-xs text-text-muted font-medium">Ba┼şlang─▒├ğ</span>
                                        <span className="text-lg font-bold text-primary">{salon.startPrice} Ôé║</span>
                                    </div>
                                </div>
                            </Link>
                        ))}
            ...
            <div className="bg-gray-50 py-20 border-y border-gray-200">
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-end mb-10">
                        <div>
                            <h2 className="text-3xl font-display font-bold text-text-main">Pop├╝ler Kategoriler</h2>
                            <p className="text-text-secondary mt-2">─░htiyac─▒n─▒za uygun uzman─▒ kategorilere g├Âre bulun.</p>
                        </div>
                        <Link href="/?type=all" className="hidden sm:flex items-center text-primary font-bold text-sm hover:underline gap-1">
                            T├╝m├╝n├╝ G├Âr <span className="material-symbols-outlined text-sm">arrow_forward</span>
                        </Link>
                    </div>
                                ...
                                <div className="absolute inset-0 p-6 flex flex-col justify-end items-start z-10">
                                    ...
                                    <div className="h-0 group-hover:h-auto opacity-0 group-hover:opacity-100 overflow-hidden transition-all duration-300">
                                        <span className="text-xs font-bold text-primary flex items-center gap-1 uppercase tracking-wide">
                                            Ke┼şfet <span className="material-symbols-outlined text-sm">arrow_right_alt</span>
                                        </span>
                                    </div>
                                </div>
                            </Link>
        ...
        <Suspense fallback={
            <div className="min-h-screen flex items-center justify-center bg-background">
                <div className="text-center">
                    ...
                    <p className="text-text-muted text-lg">Y├╝kleniyor...</p>
                </div>
            </div>
        }>
            <HomePageContent />
