...
export default function WorkingHoursTab({ salonId }: WorkingHoursTabProps) {
    ...
    const handleInitialize = async () => {
        setLoading(true);
        try {
            const data = await SalonDataService.initializeDefaultWorkingHours(salonId);
            setHours(data);
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20 md:pb-0">
            ...
            <div className="grid grid-cols-1 gap-4 md:gap-6">
                {hours.length === 0 ? (
                    <div className="bg-white p-12 rounded-[32px] border border-border text-center">
                        <p className="text-text-muted font-bold mb-4">Henüz çalışma saati tanımlanmamış.</p>
                        <button
                            onClick={handleInitialize}
                        ...
                        >
                            <Plus className="w-5 h-5" /> Şimdi Tanımla
                        </button>
                    </div>
                ) : (
